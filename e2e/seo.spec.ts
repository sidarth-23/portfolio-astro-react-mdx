...
test.describe("SEO meta tags", () => {
  test("/en emits canonical, og:url, and og:image with siteUrl", async ({ page }) => {
    await page.goto("/en")
  ...
  test("/en/rss.xml responds 200 and contains blog post links with siteUrl", async ({ request }) => {
    const response = await request.get("/en/rss.xml")
    expect(response.status()).toBe(200)
    const body = await response.text()
    expect(body).toContain(`${siteUrl}/en/blog/stop-trusting-the-client-a-real-world-guide-to-authentication`)
  })

  test("/sitemap-0.xml responds 200 and contains siteUrl loc entries", async ({ request }) => {
