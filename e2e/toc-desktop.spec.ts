...
test.describe("Desktop TOC scrollspy", () => {
  ...
  test("transitions active links, follows latest active link in container, and does not mutate hash on scroll", async ({
    ...
    await expect(
      page.locator(`[data-toc='desktop'] [data-toc-link][href='#${firstId}']`)
    ...
    await expect(
      page.locator(`[data-toc='desktop'] [data-toc-link][href='#${lastId}']`)
    ).toHaveClass(/text-foreground/)
      ...
      .poll(async () => {
        return await page.evaluate((id) => {
          const container = document.querySelector<HTMLElement>(
            "[data-toc='desktop'] [data-toc-scroll-container]"
          )
          const link = document.querySelector<HTMLElement>(
            `[data-toc='desktop'] [data-toc-link][href='#${id}']`
          )
