...
test.describe("Mobile TOC scrollspy", () => {
  ...
  test("updates active state reliably and only autoscrolls list when panel is open", async ({
    ...
    if (!firstId || !secondId || !lastId)
      throw new Error("Expected heading ids")
      ...
      .poll(async () => {
        return await page.evaluate((id) => {
          const link = document.querySelector<HTMLElement>(
            `[data-toc='mobile'] [data-toc-link][href='#${id}']`
          )
    ...
    const closedScrollTop = await page.evaluate(() => {
      const container = document.querySelector<HTMLElement>(
        "[data-toc='mobile'] [data-toc-scroll-container]"
      )
      ...
      .poll(async () => {
        return await page.evaluate(() => {
          ...
          return links.some((link) =>
            link.classList.contains("text-foreground")
          )
        })
      })
      .toBe(true)
      ...
      .poll(async () => {
        return await page.evaluate(() => {
          ...
          const activeIndexes = links
            .map((link, index) => ({
              index,
              active: link.classList.contains("text-foreground"),
            }))
          ...
          const node = document.querySelector<SVGCircleElement>(
    ...
    await page
      .locator(`[data-toc='mobile'] [data-toc-link][href='#${secondId}']`)
      .click()
